...
class MainActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        ...
        val prefs = getSharedPreferences("sensor_labels", MODE_PRIVATE)
        val nameViews = listOf(
            textViewName0 to "Peak",
            textViewName1 to "Main rotor",
            textViewName2 to "Main blades",
            textViewName3 to "Tail rotor",
            textViewName4 to "Tail shaft",
            textViewName5 to "Ventilator",
            textViewName6 to "Motor"
        )
        nameViews.forEachIndexed { index, (textView, defaultLabel) ->
            val key = "name$index"
            ...
            textView.setOnLongClickListener {
                ...
                AlertDialog.Builder(themedContext)
                    ...
                    .setPositiveButton("Save") { _, _ ->
                        val newName = editText.text.toString().ifBlank { defaultLabel }
                        textView.text = newName
                        prefs.edit().putString(key, newName).apply()
                    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            R.id.menu_start_record -> {
                //logFile.start()
                // Menü gomb cseréje
                item.isVisible = false
                menuRef.findItem(R.id.menu_stop_record)?.isVisible = true
                true
            }

            R.id.menu_stop_record -> {
                //logFile.stop()
                item.isVisible = false
                menuRef.findItem(R.id.menu_start_record)?.isVisible = true
                true
            }

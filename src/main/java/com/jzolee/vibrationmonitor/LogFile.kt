...
/*class LogFile private constructor(
    ...
    companion object {
        fun create(
            ...
            return LogFile(activity, logCallback, permissionOK).apply {
                initialize()
            }
        }
    }
    ...
    private var outputUri: Uri? = null
    private lateinit var documentLauncher: ActivityResultLauncher<Intent>
    private var pendingStart = false // ha SAF miatt el kell halasztani a start()-ot
    private lateinit var permissionLauncher: ActivityResultLauncher<String>

    private var writer: BufferedWriter? = null //private var writer: FileWriter? = null
    private var isRecording = false
        ...
        documentLauncher = activity.registerForActivityResult(
            ...
            if (result.resultCode == AppCompatActivity.RESULT_OK) {
                ...
                outputUri?.let {
                    ...
                    logCallback("SAF directory selected: $it")
                }

                if (pendingStart) {
                    pendingStart = false
                    start() // újraindítjuk start-ot, ha SAF miatt halasztva volt
                }
            } else {
                logCallback("No folder selected - recording cancelled")
            }
        }

        // Engedélyek ellenőrzése Android verzió szerint
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
            if (ContextCompat.checkSelfPermission(activity, Manifest.permission.WRITE_EXTERNAL_STORAGE)
                == PackageManager.PERMISSION_GRANTED) {
                permissionOK()
    ...
    fun start() {
        ...
        try {
            writer = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
                // SAF elérés csak akkor, ha még nincs kiválasztva
                if (outputUri == null) {
                    logCallback("Requesting SAF directory before starting recording")
                    pendingStart = true
                    requestSAFDirectory()
                ...
                val docUri = DocumentsContract.createDocument(
                    resolver, outputUri!!, "text/csv", fileName
                )

                if (docUri == null) {
                    logCallback("Failed to create document in SAF folder")
                    return
                }

                BufferedWriter(OutputStreamWriter(resolver.openOutputStream(docUri)))
            } else {
                // Klasszikus fájl mentés
                val outputFile = File(outputDirectory, fileName).apply {
                    createNewFile()
                }
                logCallback("File created at: ${outputFile.absolutePath}")
    ...
    fun start() {
        ...
        try {
            val timestamp = SimpleDateFormat(
                ...
                .format(Date(System.currentTimeMillis()))
            val outputFile = File(outputDirectory, "VM_data_${timestamp}.csv").apply {
                createNewFile()
            }
